...
## 仓库说明
🚀 京东云亚瑟 (IPQ60xx) 1GB 硬改专用 | 满血 NSS + Docker 终极版
⚠️ 注意：本固件专为硬改 1GB 内存的亚瑟 (JDCloud RE-SS-01) 打造，未改内存的原厂 512M 设备请勿刷入！
本固件基于 ImmortalWrt 源码深度优化，集成 Qualcomm 闭源 NSS 驱动，彻底释放 IPQ60xx 性能潜力。剔除冗余开发工具，专注于网络转发性能与容器化扩展能力。
🌟 核心亮点
1GB 内存解锁：启用 MEM_PROFILE_1024，完整映射 1GB 物理内存，专为大内存环境优化的 WiFi 缓冲区配置。
NSS 满血硬解：集成 qca-nss-ecm 全家桶。路由转发、桥接、PPPoE、VLAN、WiFi 数据包全部由 NPU 硬件接管，跑满千兆 CPU 占用率近乎 0%。
Docker 容器平台：内置完整 Docker 引擎 (dockerd) 及图形化管理 (dockerman)。利用 1GB 内存优势，可稳定运行 HomeAssistant、Python 脚本、Alist 等容器，玩法无限。
⚡ 性能与加速
...
磁盘管理专家：
luci-app-diskman：磁盘挂载与管理。
luci-app-partexp：一键分区扩容（刷机后轻松利用剩余空间）。
parted + smartmontools：专业分区工具与硬盘健康监控 (S.M.A.R.T)。
🔌 接口与扩展
USB 网卡/模组霸王餐：内置几十种 USB 网卡驱动，支持 RNDIS、NCM、QMI 协议，兼容主流 4G/5G 模组及 RTL8152/AX88179 USB 网卡。
外设支持：支持 USB 打印机共享 (p910nd)、USB 摄像头 (uvc)、USB 转串口 (CH341/CP210x 等)。
🛠️ 实用插件与工具
网络增强：MosDNS、HomeProxy、Passwall、OpenClash、UPnP。
...
- 如需修改默认 IP、添加或删除插件包以及一些其他设置请在 `.sh` 文件内修改。
